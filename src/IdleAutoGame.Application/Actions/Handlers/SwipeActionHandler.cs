...
/// <summary>
/// Executes a swift swipe gesture between normalized coordinates.
/// </summary>
public sealed class SwipeActionHandler : IActionHandler
{
    public ActionType SupportedAction => ActionType.Swipe;
    ...
    public async Task<ActionResult> ExecuteAsync(ActionExecutionContext context, CancellationToken ct)
    {
        ...
        {
            return ActionResult.RejectedResult(SupportedAction, "Missing start (X, Y) or end (EndX, EndY) coordinates for Swipe.");
        }
        ...
        int durationMs = p.DurationMs.HasValue && p.DurationMs.Value >= context.Settings.Automation.MinSwipeDurationMs
            ? Math.Min(p.DurationMs.Value, context.Settings.Automation.MaxSwipeDurationMs)
            : context.Settings.Automation.DefaultSwipeDurationMs;
        ...
        try
        {
            ...
            {
                return ActionResult.CancelledResult(SupportedAction, "Swipe cancelled due to engine state change.");
            }
            ...
            return ActionResult.Successful(
                SupportedAction,
                $"Swiped ({x1}, {y1}) -> ({x2}, {y2}) in {durationMs}ms",
                sw.Elapsed);
        }
        catch (OperationCanceledException)
        {
            return ActionResult.CancelledResult(SupportedAction, "Swipe cancelled by user or timeout.", sw.Elapsed);
        }
        catch (Exception ex)
        {
